...
class DataPipeline:
    ...
    def load_sessions(
        ...
        try:
            ...
            self.logger.info(f"Successfully loaded {len(df)} sessions from {len(df['site_id'].unique())} sites")
            return df
    ...
    def aggregate_hourly(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            hourly_df.columns = ['site_id', 'hour', 'sessions', 'session_count', 'avg_sessions', 'first_timestamp']
    ...
    def _fill_missing_hours(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.set_index(['site_id', 'hour']).reindex(complete_index, fill_value=0).reset_index()
    ...
    def get_data_summary(self, df: pd.DataFrame) -> dict:
        ...
        summary = {
            ...
            'date_range': {
                'start': df['timestamp'].min().isoformat() if 'timestamp' in df.columns else df['hour'].min().isoformat(),
                'end': df['timestamp'].max().isoformat() if 'timestamp' in df.columns else df['hour'].max().isoformat()
            },
            'sessions_stats': {
...
def load_sessions(path: str = 'data/raw/synthetic_sessions.csv', parse_dates: List[str] = None) -> pd.DataFrame:

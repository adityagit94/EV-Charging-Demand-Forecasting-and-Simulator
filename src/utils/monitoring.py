...
class DataDriftDetector:
    ...
    def detect_drift(self, new_data: pd.DataFrame) -> Dict:
        ...
        for column in numeric_columns:
            ...
            if (ks_p_value < self.significance_level or 
                mw_p_value < self.significance_level):
        ...
        drift_results['summary'] = {
            ...
            'drift_percentage': len(drift_results['features_with_drift']) / len(numeric_columns) * 100
        }
        
        if drift_results['drift_detected']:
            self.logger.warning(f"Data drift detected in features: {drift_results['features_with_drift']}")
        else:
...
class ModelPerformanceMonitor:
    ...
    def get_performance_summary(self, days: int = 30) -> Dict:
        ...
        summary = {
            'period_days': days,
            'total_predictions': sum(record['sample_size'] for record in recent_records),

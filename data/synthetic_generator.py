...
def generate_synthetic_data(n_sites=10, n_days=90, freq='H', seed=42,
                            out_path='data/raw/synthetic_sessions.csv'):
    np.random.seed(seed)
    ...
    for site in range(1, n_sites+1):
        ...
        daily = 3 * (np.sin((hours.hour - 7) / 24 * 2 * np.pi) + 1) +                 2 * (np.sin((hours.hour - 18) / 24 * 2 * np.pi) + 1)
        # weekly modifier
        weekly = np.where(hours.dayofweek < 5, 1.0, 0.7)
        noise = np.random.normal(0, 1.0, size=periods)
        values = np.maximum(0, base * weekly + daily + noise + np.random.normal(0,1,periods))

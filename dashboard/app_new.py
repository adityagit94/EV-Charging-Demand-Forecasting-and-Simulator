...
# Main title
st.markdown(
    '<h1 class="main-header">🚗⚡ EV Charging Demand Forecast Dashboard</h1>',
    unsafe_allow_html=True,
)
...
if not st.session_state.data_loaded or st.session_state.processed_data is None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Overview", "🔍 Site Analysis", "🗺️ State Analysis", "📊 Features"]
    )
    ...
    with tab4:
        ...
        # Feature correlation
        feature_cols = [
            col for col in hourly.columns if col not in ["site_id", "hour", "sessions"]
        ]
        feature_data = hourly[feature_cols + ["sessions"]].select_dtypes(
            include=["number"]
        )

...
def show_feature_analysis(data: pd.DataFrame) -> None:
    ...
    metrics = {
        ...
        "Temporal Features": len(
            [
                col
                for col in data.columns
                if any(x in col for x in ["hour", "day", "month", "year"])
            ]
        ),
        "Rolling Features": len(
            [
                col
                for col in data.columns
                if "r" in col and any(x in col for x in ["mean", "std", "min", "max"])
            ]
        ),
    ...
    for type_name in selected_types:
        ...
        selected_cols.extend(
            [col for col in data.columns if any(pattern in col for pattern in patterns)]
        )

    if selected_cols:
        # Ensure unique columns and add sessions if not already included
        selected_cols = list(
            dict.fromkeys(selected_cols)

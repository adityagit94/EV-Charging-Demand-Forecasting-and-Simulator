...
from components.india_visualizations import (
    create_india_choropleth,
    prepare_station_data,
    create_demand_by_region,
...
def get_state_metrics(data: pd.DataFrame) -> dict:
    ...
    return {
        ...
        f"Current Sessions ({current_hour})": data[data["hour"].dt.strftime("%H:00") == current_hour]["sessions"].mean(),
        "Avg Daily Sessions": data.groupby(data["hour"].dt.date)["sessions"].mean().mean()
    }
...
def show_state_analysis(data: pd.DataFrame) -> None:
    ...
    if "state" in data.columns:
        ...
        state_data.columns = ["state", "total_stations", "mean_sessions", "peak_sessions"]
        ...
        if len(date_range) == 2:
            ...
            mask = (data["hour"].dt.date >= start_date) & (data["hour"].dt.date <= end_date)
            ...
            daily_data = (
                filtered_data.groupby([
                    filtered_data["hour"].dt.date,
                    "state"
                ])["sessions"]
                .sum()
                .reset_index(name="sessions")
            )
            daily_data.rename(columns={filtered_data["hour"].dt.date.name: "date"}, inplace=True)
        ...
        st.warning("State information not available in the dataset. Please ensure your data includes state-level information.")
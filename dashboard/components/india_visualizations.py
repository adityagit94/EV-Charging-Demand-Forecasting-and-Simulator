...
def load_india_geojson() -> gpd.GeoDataFrame:
    ...
    def _load_geojson():
        ...
        return gpd.read_file("https://raw.githubusercontent.com/Subhash9325/GeoJson-Data-of-Indian-States/master/Indian_States")
    ...
    if show_stations and station_data is not None:
        fig.add_trace(go.Scattergeo(
            ...
            text=station_data.apply(
                lambda x: f"{x['name']}<br>Status: {x['status']}", 
                axis=1
            ),
            mode='markers',
            marker=dict(
